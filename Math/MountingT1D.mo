...
model MountingT1D "Propagate 1-dim. support flange to 3-dim. system (provided world.driveTrainMechanics3D=true)"
  ...
  Modelica.Mechanics.Translational.Interfaces.Flange_b flange_b "(right) flange fixed in housing" annotation (Placement(transformation(
          extent={{110,10},{90,-10}})));
  Modelica.Mechanics.MultiBody.Interfaces.Frame_a frame_a if world.driveTrainMechanics3D "Frame in which housing is fixed (connector is removed, if world.driveTrainMechanics3D=false)" annotation (Placement(transformation(
        origin={0,-100},
        extent={{-20,-20},{20,20}},
        rotation=90)));
protected
  ...
  encapsulated model Housing
    import Modelica;
    input Modelica.SIunits.Force f[3];
  ...
  Housing housing(f=-n*flange_b.f) if world.driveTrainMechanics3D
